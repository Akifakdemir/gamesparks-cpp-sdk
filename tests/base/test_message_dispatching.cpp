...
TEST(MessageDispatchingTest, Test01)
{
	...
	gs.SetMessageListener<AchievementEarnedMessage>([&](GS_& gs, const AchievementEarnedMessage& message) {
		std::clog << "message received" << std::endl;
		messageDispatched = true;
	});

    gs.GameSparksAvailable = [&](GameSparks::Core::GS_& gsInstance, bool avail)
    {
        isAvailable = avail;
        ...
        if(avail)
        {
			...
			reg_request.Send([&](GS_& gs, const RegistrationResponse& reg_response) {
				registrationSucceeded = true;

				AuthenticationRequest auth_request(gs);
				auth_request.SetUserName("test_user");
				auth_request.SetPassword("test_password");
				auth_request.Send([&](GS_& gs, const AuthenticationResponse& auth_response) {
