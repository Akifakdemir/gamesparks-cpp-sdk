...
int main(int argc, const char* argv[])
{
	...
	gs.SetMessageListener<GameSparks::Api::Messages::ScriptMessage>([&platform](GS_& gs, const GameSparks::Api::Messages::ScriptMessage& message){
		if (!message.GetHasErrors())
		{
			platform.DebugMsg("Received Message from server. Durable request was sent successfully");
		}
	});
	gs.GameSparksAvailable = [&platform](GS_& gs, bool available){
		if (!available)
		{
		...
		authRequest.Send([&platform](GS_& instance, const GameSparks::Api::Responses::AuthenticationResponse& response){
